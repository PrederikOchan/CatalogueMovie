...
public class UpcomingListAdapter extends RecyclerView.Adapter<UpcomingListAdapter.ItemViewHolder> {
    private ArrayList<ResultMovieItems> mData;
    private Context mContext;
    private String imgUrl = "http://image.tmdb.org/t/p/w342";

    public UpcomingListAdapter(ArrayList<ResultMovieItems> mData, Context mContext) {
        this.mData = mData;
    ...
    public void onBindViewHolder(ItemViewHolder holder, int position) {
        final ResultMovieItems upcomingItemsModel = mData.get(position);
        holder.setModel(upcomingItemsModel);
        holder.btnDetail.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent detailIntent = new Intent(mContext, DetailMovieActivity.class);
                detailIntent.putExtra(DetailMovieActivity.EXTRA_DETAIL, upcomingItemsModel);
